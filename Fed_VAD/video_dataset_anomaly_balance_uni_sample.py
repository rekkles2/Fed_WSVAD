        ...
        if self.args.larger_mem:
            if self.train:
                ...
                for a_i, n_i in zip(anomaly_indexs, normaly_indexs):
                    ...
                    anomaly_feature, r = utils.process_feat_sample1(anomaly_feature, self.t_max)
                    start_index += r
                    ...
                    normaly_feature, r = utils.process_feat1(normaly_feature, self.t_max, self.args.sample_step)
                    start_index += r
                ...
                if args.label_type == 'binary':
                    normaly_label = torch.cat(
                        (torch.ones((self.args.sample_size, 1)), torch.zeros((self.args.sample_size, 1))), dim=1)
                    anomaly_label = torch.cat(
                        (torch.ones((self.args.sample_size, 1)), torch.ones((self.args.sample_size, 1))), dim=1)
                else:
                    normaly_label = torch.cat(
                        (torch.ones((self.args.sample_size, 1)), torch.zeros((self.args.sample_size, 1))), dim=1)
                    anomaly_label = torch.cat(
                        (torch.zeros((self.args.sample_size, 1)), torch.ones((self.args.sample_size, 1))), dim=1)

                return [anomaly_features, normaly_features], [anomaly_label, normaly_label], [train_video_name,
                                                                                              start_index]
            else:
