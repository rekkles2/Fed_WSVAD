...
def pad1(feat, min_len):
    if np.shape(feat)[0] <= min_len:
        pad_width = (min_len - np.shape(feat)[0])
        zeros = np.zeros((pad_width, 3, 1408))
        return np.concatenate((feat, zeros), axis=0)
    else:
        return feat

def pad2(feat, min_len):
    if np.shape(feat)[0] <= min_len:
        pad_width = (min_len - np.shape(feat)[0])
        zeros = np.zeros((pad_width, 9, 3, 1408))
        return np.concatenate((feat, zeros), axis=0)
    else:
        return feat
...
def process_feat1(feat, length, step):
    if len(feat) > length:
        if step and step > 1:
            features, r = random_extract_step(feat, length, step)
            return pad(features, length), r
...
def process_feat_sample1(feat, length):
    if len(feat) > length:
            features, samples = random_perturb(feat, length)
            return features, samples
    else:
...
def anomap(predict_dict, label_dict, save_path, itr, save_root, zip=False):
    ...
    if zip:
        ...
        for k, v in predict_dict.items():
            ...
            label_np = label_dict[k][:len(v.repeat(16))]
            x = np.arange(len(predict_np))
            plt.plot(x, predict_np, color='b',  linewidth=2)
            plt.fill_between(x, label_np, where=label_np > 0, facecolor="red", alpha=0.3)
            plt.yticks(np.arange(0, 1.1, step=0.1))
            plt.xlabel('Frames')
